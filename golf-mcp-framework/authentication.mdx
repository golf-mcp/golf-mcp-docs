...
4. **Secure Processing**: Battle-tested FastMCP authentication providers
...
## Golf Utilities Integration

Golf v0.2.0 includes built-in utilities that work seamlessly with the authentication system:

```python
# Available in all tools automatically
from golf.utils import elicit, sample, get_context

# Elicitation utilities for user interaction
response = await elicit("What's your favorite color?")

# Sampling utilities for LLM interactions  
result = await sample("Explain quantum physics", model="gpt-4")

# Context utilities for accessing FastMCP context
context = get_context()
```
