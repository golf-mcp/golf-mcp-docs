...
- **Token Issuance**: Issues JWT tokens to clients
- **OAuth 2.0 Endpoints**: Authorization, token, and metadata endpoints
- **Token Revocation**: Supports token revocation (RFC 7009)
- **Dynamic Client Registration**: Disabled by default for security
- **Standards Compliance**: Full RFC 6749 OAuth 2.0 implementation

### Security Features

- **HTTPS Enforcement**: Warns/blocks HTTP URLs in production
- **Scope Validation**: Ensures scopes follow OAuth 2.0 format
- **Localhost Prevention**: Blocks localhost URLs in production environments
- **Client Registration Disabled**: Prevents unauthorized client registration
...
Golf v0.2.0 significantly expanded environment variable support across all auth providers, enabling proper 12-factor app configuration patterns for production deployments.
...
JWT_PUBLIC_KEY="-----BEGIN PUBLIC KEY-----
MIIBIjANBgkqhkiG9w0BAQEF..."
JWKS_URI=https://auth.production.com/.well-known/jwks.json
JWT_ISSUER=https://auth.production.com/
# Comma-separated for multiple audiences
JWT_AUDIENCE=https://api.production.com,https://api2.production.com
```

### Environment Variable Behavior

- **Runtime Resolution**: Environment variables are resolved when the provider is created, not when configured
- **Precedence**: Environment variables override static configuration values when present
- **Comma-Separated Lists**: AUTH_SERVERS and JWT_AUDIENCE support comma-separated values
- **Security Validation**: Same security rules apply to environment values (HTTPS enforcement, URL validation, etc.)
