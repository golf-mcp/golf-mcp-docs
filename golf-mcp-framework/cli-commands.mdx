...
<Note>
  Make sure to run `golf build` before using `golf run`. For automatic telemetry with Golf Platform, set `GOLF_API_KEY` before running (see [Telemetry](/golf-mcp-framework/telemetry-monitoring)).
</Note>